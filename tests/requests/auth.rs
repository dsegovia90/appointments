...
use appointments::{app::App, models::users};
...
async fn can_reset_password() {
    ...
    request::<App, _, _>(|mut request, ctx| async move {
        let login_data = prepare_data::init_user_login(&mut request, &ctx).await;
...
async fn can_get_current_user() {
    ...
    request::<App, _, _>(|mut request, ctx| async move {
        request.save_cookies();
        let prepared_data = prepare_data::init_user_login(&mut request, &ctx).await;

        let jwt_config = ctx.config.get_jwt_config().unwrap();
        let jwt = prepared_data
            .user
...
async fn can_auth_with_magic_link() {
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        // let redact_token = jsonat!("[a-zA-Z0-9]{{{}}}", users::MAGIC_LINK_LENGTH);
        // with_settings!({
        //      filters => {
...
#[tokio::test]
#[ignore = "Invalid email functionallity is turned off for now."]
#[serial]
