use appointments::{
    ...
    models::{
        ...
        users::{self, users::Role, Model, RegisterParams},
    },
};
...
async fn test_can_validate_model() {
    ...
    let invalid_user = users::ActiveModel {
        ...
        timezone: ActiveValue::set("America/Vancouver".to_string()),
        ..Default::default()
    };
...
async fn can_create_with_password() {
    ...
    let timezone = chrono_tz::Tz::America__Vancouver;

    let res = Model::create_with_password(&boot.app_context.db, &params, timezone).await;
...
async fn handle_create_with_password_with_duplicate() {
    ...
    let new_user = Model::create_with_password(
        ...
        chrono_tz::Tz::America__Vancouver,
    )
    .await;
...
async fn test_roles() {
    ...
    let params = RegisterParams {
        email: "admin@test-roles.com".to_string(),
        password: "1234".to_string(),
        name: "framework".to_string(),
    };
    ...
    let admin_user = Model::create_with_password(&boot.app_context.db, &params, timezone)
        .await
        .unwrap();

    assert_eq!(admin_user.role, Role::Admin);

    let params = RegisterParams {
        email: "normal-user@test-roles.com".to_string(),
        password: "1234".to_string(),
        name: "framework".to_string(),
    };
    ...
    let normal_user = Model::create_with_password(&boot.app_context.db, &params, timezone)
        .await
        .unwrap();

    assert_eq!(normal_user.role, Role::User);
}

#[tokio::test]
#[serial]
async fn test_register_with_settings_off() {
    configure_insta!();
    ...
    let mut admin_settings = AdminSettings::find()
        .one(&boot.app_context.db)
        .await
        .unwrap()
        .unwrap()
        .into_active_model();
    admin_settings.allow_new_registrations = ActiveValue::Set(false);
    admin_settings.update(&boot.app_context.db).await.unwrap();
    ...
    let timezone = chrono_tz::Tz::America__Vancouver;
    let user_response = Model::create_with_password(&boot.app_context.db, &params, timezone).await;
